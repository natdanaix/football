document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadSavedMatchData() {
        ...
        if (savedData) {
            ...
            if (matchState.teamA.subWindows === undefined) matchState.teamA.subWindows = calculateUsedSubWindows(matchState.teamA.substitutions);
            if (matchState.teamB.subWindows === undefined) matchState.teamB.subWindows = calculateUsedSubWindows(matchState.teamB.substitutions);
            if (!matchState.activeSubWindow) matchState.activeSubWindow = { teamA: false, teamB: false };
            updateUI();
        ...
        return `
            <div class="event-card sub-window" data-id="${window.id}">
                ...
                <div class="event-details">
                    ...
                    <div class="substitutions-list">${subsHTML}</div>
                </div>
    ...
    function showInjuryTimeSummary(currentPeriodSeconds) {
        ...
        let summaryHTML = `
            <p>เวลาทดบาดเจ็บช่วงนี้: +${currentMins}:${currentSecs}</p>
            <p style="margin-top: 8px;">เวลาทดบาดเจ็บสะสมทั้งหมด: +${totalMins}:${totalSecs}</p>
        `;
        if (matchState.injuryTimePeriods.length > 1) {
            summaryHTML += `
                <div style="margin-top: 16px;">
                    <p>ประวัติเวลาทดบาดเจ็บ:</p>
    ...
    function deleteCardEvent() {
        ...
        if (isTeamA) matchState.teamA.cards = matchState.teamA.cards.filter(card => card.id !== cardToEdit.id);
        else matchState.teamB.cards = matchState.teamB.cards.filter(card => card.id !== cardToEdit.id);
        renderTeamCards();
        saveMatchData();
        cardModal.style.display = 'none';
    }
    ...
    function resetAllData() {
        clearInterval(matchTimer);
        clearInterval(injuryTimer);
        if (matchState.isMatchStarted) exportToPDF();
        matchState = {
            isMatchStarted: false,
            startTime: null,
            elapsedTime: "00:00",
    ...
    function endMatch() {
        if (!matchState.isMatchStarted) {
            alert('กรุณาเริ่มการแข่งขันก่อน');
            return;
        }
        if (confirm('คุณต้องการจบการแข่งขันและบันทึกผลหรือไม่?')) {
            clearInterval(matchTimer);
            clearInterval(injuryTimer);
